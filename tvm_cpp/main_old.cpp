...
void RunTVMModel(const std::string& model_path, const std::string& lib_path,
                 const std::string& params_path, int device_type,
                 int device_id) {
  ...
  std::iota(input_data.begin(), input_data.end(),
            0.0f);  // Example: fill with increasing floats
  ...
  if (output_tensor.device.device_type != kDLCPU) {
    DLTensor cpu_output_tensor =
        output_tensor;  // Create a DLTensor pointing to CPU memory
...
int main() {
  ...
  RunTVMModel(model_json_path, model_lib_path, model_params_path, device_type,
              device_id);
